...
const getStored = (key, fallback) => { try { return repairData(JSON.parse(localStorage.getItem(key))) || fallback } catch { return fallback } }
  ...
  useEffect(() => {
    ...
    request
      .then(({ data, error }) => {
        ...
        if (!cancelled && data) {
          setProfile({ ...repairData(data.profile), email: 'huyenkim2506@gmail.com' })
...
function PublicPage({ profile, links, theme, fontFamily, onClick }) { return <div className={'public-page '+theme+' font-'+fontFamily}><div className="public-card"><div className="public-menu">•••</div><div className="public-avatar">{profile.avatar ? <img src={profile.avatar} alt={profile.name} style={{ objectPosition: `${profile.avatarX ?? 50}% ${profile.avatarY ?? 50}%`, transform: `scale(${profile.avatarZoom ?? 1})` }}/> : profile.name.split(' ').map((x) => x[0]).slice(-2).join('')}</div><h1>{profile.name}</h1><p>{profile.bio}</p><div className="socials"><span>f</span><span>◎</span><span>▶</span></div><div className="public-links">{links.map((link) => <button className="public-link" key={link.id} onClick={() => { onClick(link); if (link.url.startsWith('http')) window.open(link.url, '_blank', 'noopener,noreferrer') }}><LinkVisual link={link} className="phone-link-icon"/><strong>{link.title}{link.price && <small className="public-price">{link.price}</small>}</strong><MoreHorizontal size={17}/></button>)}</div><div className="public-brand">✦ biogen</div></div></div> }
