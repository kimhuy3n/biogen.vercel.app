...
function App() {
  ...
  return <div className="app-shell"><FontPicker value={fontFamily} onChange={setFontFamily}/><ShopTools onAdd={() => setLinkEditor({ type: 'link', category: 'shop', title: '', url: '', icon: '🛍', color: '#ff6b35' })}/><AccountTools email={session?.user?.email} onLogout={() => supabase?.auth.signOut()}/><SecurityTools email={session?.user?.email}/>
