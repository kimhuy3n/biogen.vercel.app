...
function PublicPage({ profile, links, theme, onClick }) { return <div className={'public-page '+theme}><div className="public-card"><div className="public-menu">•••</div><div className="public-avatar">{profile.avatar ? <img src={profile.avatar} alt={profile.name}/> : profile.name.split(' ').map((x) => x[0]).slice(-2).join('')}</div><h1>{profile.name} <span className="verified">✓</span></h1><p>{profile.bio}</p><div className="socials"><span>f</span><span>◎</span><span>▶</span></div><div className="public-links">{links.map((link) => <button className="public-link" key={link.id} onClick={() => { onClick(link); if (link.url.startsWith('http')) window.open(link.url, '_blank', 'noopener,noreferrer') }}><span className="phone-link-icon" style={{ background: link.color }}>{link.icon}</span><strong>{link.title}</strong><MoreHorizontal size={17}/></button>)}</div><div className="public-brand">✦ biogen</div></div></div> }
